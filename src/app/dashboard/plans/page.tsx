  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex items-center justify-between">
        ...
        {!isAdmin && <span className="text-xs bg-muted px-2 py-1 rounded">View only • Staff cannot edit</span>}
      </div>

      <div className={`grid gap-4 sm:gap-6 ${isAdmin ? "grid-cols-1 lg:grid-cols-3" : "grid-cols-1"}`}>
        {isAdmin && (
        ...
        <Card className={isAdmin ? "lg:col-span-2 overflow-hidden" : "overflow-hidden"}>
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Price</TableHead><TableHead>Duration</TableHead><TableHead>Type</TableHead><TableHead>Status</TableHead>{isAdmin && <TableHead></TableHead>}</TableRow></TableHeader>
              <TableBody>
                {plans?.map((p: any) => (
                  <TableRow key={p.id}>
                ...
                {(!plans || plans.length === 0) && <TableRow><TableCell colSpan={isAdmin ? 6 : 5} className="text-center py-8 text-muted-foreground">No plans yet.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </div>
        </Card>
